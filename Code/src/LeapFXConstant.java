/**
 * Created by liquidsunset on 18.05.16.
 */
class LeapFXConstant {

    static final boolean FREE_MODE = false;
    static final boolean DIRECT_LINE_MODE = false;
    static final boolean MIRROR_MAIN_VIEW = false;
    static final boolean MIRROR_SUB_VIEW = false;
    static final boolean SHOW_SUBVIEW = false;
    static final boolean OVERWRITE_SEQUENCE = true;
    static final String FILE_SEQUENCE_NAME = "SEQUENCE.txt";
    static final String LOG_NAME = "logForTest";
    static final int SEQUENCE_LENGTH = 9;
    static final int TEXT_SIZE = 50;
    static final int COUNT_ELEMENTS = 3;
    static final double HUD_WIDTH = 800;
    static final double HUD_HEIGHT = 200.0;
    static final long TIME_OUT_IN_MS = 1500;
    static final int DOUBLE_ROUND_DECIMAL = 2;
    static final double[] ANGELS_THREE_ELEMENTS = {180.0, 100.0, 80.0, 0.0};
    static final double[] ANGELS_FOUR_ELEMENTS = {180.0, 100.0, 95.0, 85.0, 0.0};
    static final double[] ANGELS_FIVE_ELEMENTS = {180.0, 115.0, 100.0, 85.0, 75.0, 0.0};
    static final double[] ANGELS_THREE_ELEMENTS_DIRECT = {180.0, 100.0, 85.0, 0.0};
    static final double[] ANGELS_FOUR_ELEMENTS_DIRECT = {180.0, 105.0, 95.0, 85.0, 0.0};
    static final double[] ANGELS_FIVE_ELEMENTS_DIRECT = {180.0, 120.0, 105.0, 95.0, 80.0, 0.0};

}
